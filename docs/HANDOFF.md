...
**일반적인 브랜치 → PR → 머지 흐름을 따른다. `main` 직접 push 하지 않는다.**

```
feat/살아있는-방   ← 작업 브랜치. 커밋은 작게, 자주. push 도 자유(백업 + CI)
        ↓ PR
      main         ← 출시 단위. 기능이 끝나고 검증된 뒤 한 번
```

- **브랜치에서 작업한다.** `feat/` `fix/` `docs/` 접두어. 커밋은 작은 단위로 자유롭게 쪼갠다 — 리뷰와 되돌리기가 쉬워진다.
- **브랜치 push 는 마음껏.** 백업이 되고 CI 가 돌아간다. 배포는 만들지 않는다.
- **`main` 머지는 "출시"다.** 기능 하나가 끝나고 검증된 시점에만. 문서 한 줄, 오타 하나로 머지하지 말고 다음 작업에 묶는다.
- **`main` 에 직접 push 금지.** 2026-09-09 에 커밋마다 main 으로 push 해서 하루에 12회 배포(180크레딧, 월 예산의 60%)를 태웠다. 브랜치를 쓰면 구조적으로 막힌다.
- 부수적으로 비용도 맞아떨어진다 — `main` 에 올라간 것이 곧 프로덕션 배포 1회 = **15크레딧**, Free plan 월 20회가 상한이다(§8-1 실측).

**검증 3단계** (아래로 갈수록 비싸니 위에서 최대한 잡는다):
...
- 이 repo 작업은 Claude Code 에서 repo 바운드로 한다. (참고: Cowork 세션에서는 git 루프가 막혀 있어 브라우저 우회가 필요했고, 그래서 Claude Code 로 이관했다.)
