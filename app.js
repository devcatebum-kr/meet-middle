(() => {
  ...
  function boot() {
    ...
    // 카카오 공유(카톡 초대)용 SDK — 실패해도 공유는 폴백으로 동작
    const ks = document.createElement("script");
    ks.src = "https://t1.kakaocdn.net/kakao_js_sdk/2.7.4/kakao.min.js";
    ks.onload = () => { try { if (window.Kakao && !Kakao.isInitialized()) Kakao.init(key); } catch (e) {} };
  ...
  async function makeRoom() {
    try {
      ...
      enterRoom(d.roomId, false);
    } catch (e) {
      toast("방 생성에 실패했어요. 잠시 후 다시 시도해주세요.");
    }
  }

  function enterRoom(roomId, invited) {
    show("roomView");
    S.room = { roomId, participants: [] };
    S.myPid = localStorage.getItem("pid:" + roomId) || null;
    $("inviteBanner").hidden = !invited;

    // 공유용 링크는 /s (OG 카드 서버렌더) — 열면 앱(?room=)으로 리다이렉트됨
    const link = location.origin + "/s?room=" + roomId;
    $("shareLink").value = link;
    $("copyLink").onclick = () =>
      navigator.clipboard.writeText(link).then(() => toast("링크를 복사했어요!"), () => toast(link));
    $("inviteBtn").onclick = () => shareInvite(link);
